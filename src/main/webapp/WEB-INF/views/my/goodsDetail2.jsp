	...
	<div style="width:100%; align:center; height:600px;">
		...
		<div style="float:left; margin-left:350px; width:26%;">
			<table border="0" width="500">
				<tr>
					<td><img src="/base/file/${list.GOODS_THUMBNAIL }" width="500"
						height="500" /></td>
				</tr>
			</table>
		</div>
...
function fn_Qna(){ // 상품문의 작성
	...
	if(${SESSION_NO ne null}){
		...
		alert("로그인 후 이용해주세요.");
		location.href = "/base/loginForm.do";
	}
...
function fn_InsertBasket() { // 장바구니
	...
	if(${SESSION_NO ne null}){
		...
		if(len==0){
			...
			var url = "/base/shop/basketPopUp.do";
			var name = "popup";
...
function fn_selectGoodsListCallback(data) {
	...
	if (total == 0) {
		...
		$.each(data.list, function(key, value) {
							    ...
							    var img = "<img src='/base/img/ico_encryption.png'>";	
								var Q= "<img src='/base/img/ico_qna_q.png'>";
								var A= "<img src='/base/img/ico_qna_a.png'>";
								if(Level == 1){//답변
									if(sc == 0){//공개
							str += "  "
								+  " <tr id='show"+cnt1+"'> "
...
function fn_selectReviewListCallback(data) {
	...
	if (total == 0) {
		...
		$.each(data.reviewList, function(key, value) {
							...
							var img = "<img src='/base/img/icon_201602021908415400.jpg' >"
							var REVIEW_IMG = value.REVIEW_IMG;
							var gubun = value.REVIEW_NO;
							var imgpath = "<img src='/base/file/"+value.REVIEW_IMG+"' width='500' height='400'>";
