...
<!-- 합쳐지고 최소화된 최신 자바스크립트 -->
<script src="/base/js/jquery-3.0.0.min.js"></script>
...
function order_ok(mem_no, order_no){
	...
	$.ajax({
		url: "/base/order_ok.do",
		data : {"mem_no": mem_no, "order_no": order_no},
		type: "post",
		async:false,
		success : function(data){
		alert("수취확인이 완료되었습니다.");
		location.href = "/base/myOrderList.do";
		}
	})
	}else{
	return;
					...
					<c:forEach items="${my_order }" var="my_order">					
						<tr>
							...
							<c:choose>
								...
								<a href="/base/my_detail.do?order_no=${my_order.ORDER_NO  }" name="title">${my_order.GOODS_NAME }</a>
								<input type="hidden" id="member_no" value="${my_order.MEMBER_NO }">
								</c:when>
								<c:otherwise>
								<a href="/base/my_detail.do?order_no=${my_order.ORDER_NO  }" name="title">${my_order.GOODS_NAME }</a>외 ${my_order.HAP_CNT -1 }건
								<input type="hidden" id="member_no" value="${my_order.MEMBER_NO }">
								</c:otherwise>
							</c:choose>
							</td>
